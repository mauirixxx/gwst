...
GWST is a PHP application backed by MariaDB/MySQL. These instructions assume you already have a working web server with PHP and MariaDB/MySQL available.
...
```bash
git clone https://github.com/mauirixxx/gwst.git gwst
cd gwst
```
...
```sql
CREATE DATABASE gwst;

CREATE USER 'gwstuser'@'localhost'
  IDENTIFIED BY 'PUT_A_STRONG_PASSWORD_HERE';

GRANT ALL PRIVILEGES ON gwst.* TO 'gwstuser'@'localhost';
```

GWST does not need to connect to the database as `root`.
...
## 5. Open GWST and register the first user

Browse to the URL where you installed GWST and create an account.

On a fresh database, the **first successfully registered GWST user is automatically granted administrator access**. No default administrator username or password is shipped with GWST. All later registrations are normal non-administrator accounts.
...
For the first Guild Wars account, GWST suggests the e-mail address used to register your GWST account. You may keep it, replace it with the actual Guild Wars account e-mail, or replace it with an alias.

The first Guild Wars account you add is automatically selected as your preferred account. You can then begin adding characters and tracking title progress.
