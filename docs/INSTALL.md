...
## 7. Optional public-web and Cloudflare hardening

GWTTT does not require Cloudflare or Apache-specific hardening to operate. For a public deployment, however, optional defense-in-depth guidance is available in [OPTIONAL-WEB-HARDENING.md](OPTIONAL-WEB-HARDENING.md).

That guide covers Apache session/security headers, denying direct web access to configuration, Git, SQL, CLI scripts and Composer dependencies, and optional Cloudflare use. If Cloudflare proxies the site, configure Apache `mod_remoteip` so PHP's `REMOTE_ADDR` contains the real visitor address; only trust `CF-Connecting-IP` from Cloudflare's published proxy networks. This matters because GWTTT's authentication throttling uses `REMOTE_ADDR`.

Cloudflare WAF, bot protection and rate limiting can provide another layer of protection, but they should complement rather than replace GWTTT's application-level authentication, authorization, CSRF and validation controls.
