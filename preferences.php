...
if (isset($_SESSION['userid'])){
    ...
    if (!empty($_POST['useremail'])) include_once ('includes/update-email.php');
    if (!empty($_POST['oldpass'])) include_once ('includes/update-password.php');
    if (!empty($_POST['setacc'])) include_once ('includes/set-prefacc.php');
    if (!empty($_POST['setchar'])) include_once ('includes/set-prefchar.php');

    echo '<h3>Set preferred account &amp; character, or change e-mail or password</h3>';
    echo '<fieldset class="options-card account-options-card"><legend>Account &amp; character</legend>';

    echo '<form action="preferences.php" method="post" class="options-form">' . csrf_input();
    echo '<div class="options-current">Current preferred account: <strong>' . h($_SESSION['prefaccname']) . '</strong></div>';
    echo '<div class="options-control-row"><select name="prefaccid"><option value="nopref">Prefer no default</option>';
    ...
    while ($row = $resacc->fetch_assoc()) echo '<option value="' . (int)$row['accid'] . '">' . h($row['accemail']) . '</option>';
    $prefacc->close();
    echo '</select><button type="submit">Set account</button></div><input type="hidden" name="setacc" value="update"></form>';

    echo '<form action="preferences.php" method="post" class="options-form">' . csrf_input();
    echo '<div class="options-current">Current preferred character: <strong>' . h($_SESSION['prefcharname']) . '</strong></div>';
    echo '<div class="options-control-row"><select name="prefcharid"><option value="nopref">Prefer no default</option>';
    ...
    while ($row2 = $reschar->fetch_assoc()) echo '<option value="' . (int)$row2['charid'] . '">' . h($row2['charname']) . '</option>';
    $prefchar->close();
    echo '</select><button type="submit">Set character</button></div><input type="hidden" name="setchar" value="updatechar"></form>';

    echo '<form action="preferences.php" method="post" class="options-form options-email-form">' . csrf_input();
    echo '<label for="useremail">Update e-mail address</label>';
    echo '<div class="options-control-row"><input id="useremail" type="email" name="useremail" maxlength="50" value="' . h($_SESSION['usermail']) . '" required><button type="submit">Update e-mail</button></div></form>';
    echo '</fieldset>';
    ...
    echo '<form action="preferences.php" method="post">' . csrf_input();
    echo '<fieldset class="birthday-preferences"><legend>E-mail reminders</legend>';
    ...
    echo '<input type="hidden" name="save_email_preferences" value="1"><button type="submit">Save e-mail preferences</button>';
    echo '<p class="birthday-help">Birthday and treasure reminders are disabled unless you explicitly opt in.</p></fieldset></form>';
    if ($preference_message !== '') echo '<p><strong>' . h($preference_message) . '</strong></p>';

    echo '<fieldset class="options-card password-options-card"><legend>Change password</legend>';
    echo '<form action="preferences.php" method="post" class="password-options-form">' . csrf_input();
    echo '<label for="oldpass">Current password</label><input type="password" name="oldpass" id="oldpass" autocomplete="current-password" required>';
    echo '<label for="up1">New password</label><input type="password" name="userpass1" id="up1" minlength="8" maxlength="255" autocomplete="new-password" required>';
    echo '<label for="up2">Verify new password</label><input type="password" name="userpass2" id="up2" minlength="8" maxlength="255" autocomplete="new-password" required>';
    echo '<button type="submit" name="submission" id="btnSubmit">Update password</button></form></fieldset>';
?>
<script type="text/javascript">
document.getElementById("btnSubmit").addEventListener("click", function (event) {
    var userpass1 = document.getElementById("up1").value;
    var userpass2 = document.getElementById("up2").value;
    if (userpass1 !== userpass2) { event.preventDefault(); alert("Passwords do not match."); }
});
</script>
<style>
