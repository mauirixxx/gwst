...
if (isset($_SESSION['userid'])){
    ...
    if (($_SESSION['admin'] ?? 0) == 1) {
        if (isset($_POST['title'])) {
            if ($_POST['title'] == "addtitle") include_once ('includes/title-add.php');
            else if ($_POST['title'] == "titlesubmit") include_once ('includes/title-submit.php');
            else if ($_POST['title'] == "modtitle") include_once ('includes/title-editor.php');
            else if ($_POST['title'] == "updatetitle") include_once ('includes/title-update.php');
            else if ($_POST['title'] == "addsubtitle") include_once ('includes/titleranks-add.php');
            else if ($_POST['title'] == "titleranksubmit") include_once ('includes/titleranks-submit.php');
            else if ($_POST['title'] == "modsubtitle") include_once ('includes/titleranks-editor.php');
            else if ($_POST['title'] == "updatesubtitle") include_once ('includes/titleranks-update.php');
        } else {
            unset($_SESSION['tid']);
            unset($_SESSION['tr']);
?>
...
<div class="title-manager-heading"><h1>Title Manager</h1><p>Add and maintain Guild Wars titles, ranks, and point requirements.</p></div>
<div class="title-manager-tools">
<section class="title-manager-card"><h2>Add title</h2><p>Create a new account or character title.</p>
<form action="titlemanager.php" method="post"><?php echo csrf_input(); ?><input type="hidden" name="title" value="addtitle"><input class="title-manager-button" type="submit" value="Add title"></form></section>
<section class="title-manager-card"><h2>Modify title</h2><p>Edit an existing title and its settings.</p>
<form action="titlemanager.php" method="post"><?php echo csrf_input(); ?><input type="hidden" name="title" value="modtitle"><select name="tid" onchange="this.form.submit()"><option selected disabled>Select title</option><?php include ('includes/title-select.php'); ?></select><noscript><input class="title-manager-button" type="submit" value="Modify title"></noscript></form></section>
<section class="title-manager-card"><h2>Title ranks &amp; points</h2><p>Add or modify ranks and point requirements.</p>
<form action="titlemanager.php" method="post"><?php echo csrf_input(); ?><input type="hidden" name="title" value="addsubtitle"><select name="tid" onchange="this.form.submit()"><option selected disabled>Select title</option><?php include ('includes/title-select.php'); ?></select><noscript><input class="title-manager-button" type="submit" value="Manage title ranks"></noscript></form></section>
</div>
<section class="title-manager-recent"><div class="title-manager-recent-header"><h2>Recently added titles</h2><p>The 15 newest title records in the database.</p></div><div class="title-manager-table-wrap"><table class="title-manager-table"><thead><tr><th>ID</th><th>Title</th><th>Type ID</th><th>Applies to</th><th>Autofill ID</th><th>Autofilled</th></tr></thead><tbody>
<?php
$stmtview = $con->prepare("SELECT * FROM gwtitles ORDER BY titlenameid DESC LIMIT 15");
$stmtview->execute(); $result = $stmtview->get_result();
while ($row = $result->fetch_assoc()) {
    $tid=(int)$row['titlenameid']; $tname=$row['titlename']; $ttype=(int)$row['titletype']; $tmr=(int)$row['titlemaxrank']; $taf=(int)$row['autofilled'];
    if (!in_array($ttype,[0,1],true) || !in_array($taf,[0,1],true)) continue;
    $typeLabel=$ttype===0?'Account':'Character'; $autofillLabel=$taf===1?'Yes':'No'; $autofillClass=$taf===1?' yes':'';
    echo '<tr><td>'.$tid.'</td><td class="title-name">'.h($tname).' <span class="title-manager-badge">'.$tmr.' rank'.($tmr===1?'':'s').'</span></td><td>'.$ttype.'</td><td>'.h($typeLabel).'</td><td>'.$taf.'</td><td><span class="title-manager-badge'.$autofillClass.'">'.h($autofillLabel).'</span></td></tr>';
}
$stmtview->close();
?>
</tbody></table></div></section><p class="title-manager-note">If anything looks off, select the title above to fix it.</p></section>
<?php
        }
    }
}
