...
.title-form-grid { display:grid; grid-template-columns:minmax(240px,2fr) minmax(150px,1fr) minmax(120px,.7fr); gap:18px; align-items:start; }
...
.title-form-choice label { display:block; margin:7px 0; }
...
@media(max-width:700px){.title-form-grid,.title-form-flags{grid-template-columns:1fr;}}
</style>
...
<fieldset class="title-form-choice"><legend>Applies to</legend><label><input type="radio" name="titletype" value="0" checked> Account</label><label><input type="radio" name="titletype" value="1"> Character</label><small>Account titles are shared by characters on that Guild Wars account.</small></fieldset>
