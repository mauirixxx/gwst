...
.manage-button-secondary { border-color: #526b77; background: #243943; }
...
.manage-character-grid .manage-button { white-space: nowrap; }
...
<section class="manage-page">
    ...
    <fieldset class="manage-panel">
        <legend>Guild Wars accounts</legend>
        <h2 class="manage-subheading">Add a new account e-mail or alias</h2>
        <?php if ((int)$existing_account_count === 0): ?>
            <p class="manage-help">Your signup e-mail is suggested below. Keep it, replace it with your Guild Wars login e-mail, or use an alias.</p>
        <?php endif; ?>
        <form action="addaccounts.php" method="post" class="manage-add-row">
            <div class="manage-field">
                <label for="accemail">Account e-mail or alias</label>
                <input id="accemail" type="text" name="accemail" value="<?php echo h($suggested_account_name); ?>" required>
            </div>
            <input class="manage-button" type="submit" value="Add account">
        </form>
        ...
        <form action="addaccounts.php" method="post">
            <div class="manage-table-wrap">
                <table class="manage-table">
                    <thead><tr><th class="manage-id">ID</th><th>Account name</th><th class="manage-choice">Preferred</th><th class="manage-choice">Delete?</th></tr></thead>
                    <tbody>
<?php
    ...
    while ($row = $result->fetch_assoc()) {
        echo '<tr><td class="manage-id">' . (int)$row['accid'] . '</td>';
        echo '<td><span class="manage-account-link">' . h($row['accemail']) . '</span></td>';
        echo '<td class="manage-choice"><input type="radio" name="prefaccid" value="' . (int)$row['accid'] . '" aria-label="Make ' . h($row['accemail']) . ' preferred"';
        if ($row['accid'] == $_SESSION['prefaccid']) echo ' checked';
        echo '></td>';
...
?>
            ...
            <div class="manage-actions"><input class="manage-button" type="submit" value="Modify selected accounts"></div>
        </form>
    </fieldset>
...
?>
    ...
    <fieldset class="manage-panel">
        ...
        <?php if (!$selected_account): ?>
            <div class="manage-empty">Add and select a Guild Wars account before adding characters.</div>
        <?php else: ?>
            <h2 class="manage-subheading">Add character to <?php echo h($selected_account['accemail']); ?></h2>
            <form action="addaccounts.php" method="post" class="manage-character-grid">
                <div class="manage-field">
                    <label for="newcharname">Character name</label>
                    <input id="newcharname" type="text" name="newcharname" required>
                </div>
                <div class="manage-field">
                ...
                <div class="manage-field">
                    <label for="profid">Profession</label>
                    <select id="profid" name="profid" required>
<?php
        $gp = $con->prepare("SELECT profid, profession FROM gwprofessions");
        $gp->execute();
        $result = $gp->get_result();
        while ($row = $result->fetch_assoc()) {
            echo '<option value="' . (int)$row['profid'] . '">' . h($row['profession']) . '</option>';
        }
        $gp->close();
?>
                    </select>
                </div>
                <input class="manage-button" type="submit" value="Add character">
            </form>
        <?php endif; ?>
        ...
        <form action="addaccounts.php" method="post">
            <div class="manage-table-wrap">
                <table class="manage-table">
                    <thead><tr><th class="manage-id">ID</th><th class="manage-id">Account</th><th>Character name</th><th>Birthdate</th><th class="manage-choice">Preferred</th><th class="manage-choice">Delete?</th></tr></thead>
                    <tbody>
<?php
    ...
    while ($row2 = $res2->fetch_assoc()) {
        echo '<tr><td class="manage-id">' . (int)$row2['charid'] . '</td>';
        echo '<td class="manage-id">' . (int)$row2['accid'] . '</td>';
        ...
        echo '<td class="manage-choice"><input type="radio" name="prefcharid" value="' . (int)$row2['charid'] . '" aria-label="Make ' . h($row2['charname']) . ' preferred"';
        if ($row2['charid'] == $_SESSION['prefcharid']) echo ' checked';
        echo '></td>';
...
?>
            ...
            <div class="manage-actions"><input class="manage-button" type="submit" value="Modify selected characters"></div>
        </form>
    </fieldset>

    <p class="manage-return">Return to your <a href="index.php" class="navlink">user page</a></p>
</section>
<?php
}
