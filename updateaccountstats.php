...
if (isset($_SESSION['userid'])) {
    if (!isset($_POST['acctitle'])) { $_POST['acctitle'] = "notselected"; }
    ...
    if (isset($_POST['titlepoints'])) { include_once ('includes/update-titleranks.php'); }
    ...
    if ($_POST['acctitle'] == "notselected") {
        ...
        $ats->execute(); $result = $ats->get_result();
        while ($row = $result->fetch_assoc()) { echo '<option value="' . (int)$row['titlenameid'] . '">' . h($row['titlename']) . '</option>'; }
        echo '</select><button type="submit">Select title</button></div></form>'; $ats->close();
    } else {
        $selected_title = $con->prepare("SELECT titlename FROM gwtitles WHERE titlenameid = ? AND titletype = 0");
        $selected_title->bind_param("i", $_POST['acctitle']); $selected_title->execute(); $selected_title->bind_result($selected_title_name);
